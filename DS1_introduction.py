...
# 如果 a+b+c = 100,并且a^2 + b^2 = c^2,求出所有的a,b,c可能的组合
# 解决同一个问题，时间短的算法更好
...
'''
    ...
    2.算法的五大特性
        ...
        确定性：算法中的每一个步骤都有确定的含义，不会出现二义性
        可行性：算法的每一步都是可行的（每一步都能执行有限的次数完成）
    3.算法的效率衡量
        实现算法程序的执行时间可以反映出来算法的效率
        单纯的依靠运行时间来比较算法的优劣不一定是客观准确的（程序的运行离不开计算环境，所以和硬件，操作系统）
    4.最终算法用什么去衡量？
        时间复杂度
    5.表示法：大O记法
        假设计算机执行算法每个基本操作的时间是一个固定的一个时间单位，那么有多少个基本操作就代表会花费多少时间单位
        虽然对于不同的机器环境而言，确切的时间单位是不同的，但是对于算法进行多少个基本操作在规模数量级是相同的
        因此可以忽略机器环境的影响而客观的反映 算法的时间效率
        对于算法的时间效率，用‘大O记法’ 
        O(n^3)      100n^2       1000n^2
        O(n^2)
    6.时间复杂度分类
        最优时间复杂度：算法完成工作最少需要多少基本操作（过于理想化，没什么参考价值）
        最坏时间复杂度：算法完成工作最多需要多少基本操作（提供了一种保证，表名算法在此程度的基本操作中一定能完成工作）
